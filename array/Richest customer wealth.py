...
# therefore for finiding the highest sum we just use the max method in python, which returns the highest element from the list and we have our desired answer
class Solution:
    def maximumWealth(self, accounts: List[List[int]]) -> int:
        indi=[sum(p) for p in accounts]
        return max(indi)
